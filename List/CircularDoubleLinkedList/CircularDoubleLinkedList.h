...
// 함수 원형 선언
Node* CDLL_CreateNode(ElementType NewData); // 최초 노드(헤드노드) 생성
void CDLL_DestroyNode(Node* Node); // 노드 파괴(메모리 해제)
void CDLL_AppendNode(Node** Head, Node* NewNode); // 노드 추가 (테일노드에 추가)
void CDLL_InsertAfter(Node* Current, Node* NewNode); // 노드 삽입
void CDLL_RemoveNode(Node** Head, Node* Remove); // 링크드리스트에서 노드 제거
